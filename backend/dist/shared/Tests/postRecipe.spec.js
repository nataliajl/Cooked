...
let createRecipeForm = {
    ...
    private: false,
    steps: ['Colocar no fogo', 'Colocar no prato', 'Comer'],
};
beforeAll(() => {
...
describe('Post Recipe Tests', () => {
    ...
    it('Should return status 200', async () => {
        const res = await request(server_1.default).post('/recipes').send(createRecipeForm);
    ...
    it('JSON Should have value in all recipe properties ', async () => {
        ...
        const res = await request(server_1.default).post('/recipes').send(recipeForm);
